...
from apps.soil_laboratory.repositories.sample import SampleRepository, SampleLoadOptions
...
class SampleService:
    ...
    async def get_sample_by_id(self, sample_id: UUID) -> SampleDetailResponse:
        sample = await self.sample_repo.get_by_id(sample_id, include=[SampleLoadOptions.TESTS, ])
    ...
    async def get_sample_lookup_options(
        ...
        sample_entities = await self.sample_repo.get_for_lookup(
            ...
            SearchCriteria(search, Sample.molding_sand_recipe),
            OrderCriteria(Sample.received_at)
        )
        response_items = [
            SampleLookupResponse.model_validate(sample)
            for sample in sample_entities
        ]

        return response_items

    async def get_samples_paginated(self, page: int, per_page: int) -> SampleListResponse:
        total_samples = await self.sample_repo.get_count(
            where_conditions=[Sample.deleted_at == None, ]
        )
